...
export function RecordsPage() {
  ...
  const [filters,setFilters] = useState({startDate:'',endDate:'',productName:'',line:''})
  ...
  const clear = () => { setFilters({startDate:'',endDate:'',productName:'',line:''}); setLoading(true); api.list().then(setRecords).catch(e=>setError(e.message)).finally(()=>setLoading(false)) }
  ...
  return <div className="mx-auto max-w-[1500px] px-4 py-6 lg:px-8 lg:py-8">
    ...
    <section className="card mb-6 p-4 lg:p-5">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        ...
        <label><span className="label">產線</span><select className="input" value={filters.line} onChange={e=>setFilters({...filters,line:e.target.value})}><option value="">所有產線</option>{LINES.map(line=><option key={line}>{line}</option>)}</select></label>
    ...
    <section className="card overflow-hidden">
      ...
      {loading ? <div className="p-16 text-center text-slate-500">資料載入中…</div> : records.length === 0 ? <div className="p-16 text-center"><FileSpreadsheet className="mx-auto mb-3 text-slate-300" size={42}/><b className="text-slate-700">查無生產紀錄</b><p className="mt-1 text-sm text-slate-500">請調整篩選條件或新增第一筆資料</p></div> : <>
        <div className="hidden overflow-x-auto lg:block"><table className="w-full min-w-[1300px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr>{['日期','使用者','產線','品名','數量','規格摘要','預計時間','實際時間','總時數','狀態','操作'].map(x=><th className="whitespace-nowrap px-4 py-3" key={x}>{x}</th>)}</tr></thead><tbody className="divide-y">{records.map(r=><tr key={r.id} className="hover:bg-slate-50"><td className="whitespace-nowrap px-4 py-4 text-slate-600">{formatDate(r.recordDate)}</td><td className="whitespace-nowrap px-4 py-4 font-semibold">{r.userName}</td><td className="px-4 py-4"><b className="rounded bg-slate-100 px-2 py-1">{r.productionLine}</b></td><td className="max-w-48 px-4 py-4 font-bold text-slate-900">{r.productName}</td><td className="whitespace-nowrap px-4 py-4">{r.quantityMaru} 丸<br/><span className="text-slate-500">{r.quantityBox} 箱</span></td><td className="max-w-60 px-4 py-4 text-xs leading-5 text-slate-600">ID {r.specIdValue||'—'} ± {r.specIdTolerance||'—'}<br/>厚度 {r.thicknessValue||'—'} mm</td><td className="whitespace-nowrap px-4 py-4 text-xs leading-5">{formatDate(r.plannedStartTime)}<br/>{formatDate(r.plannedEndTime)}</td><td className="whitespace-nowrap px-4 py-4 text-xs leading-5">{formatDate(r.actualStartTime)}<br/>{formatDate(r.actualEndTime)}</td><td className="whitespace-nowrap px-4 py-4 font-bold">{r.totalProductionText||'—'}</td><td className="px-4 py-4"><StatusBadge status={r.status}/></td><td className="px-4 py-4"><div className="flex gap-1"><Link title="編輯" className="rounded-lg p-2 text-blue-700 hover:bg-blue-50" to={`/admin/records/${r.id}/edit`}><Edit3 size={17}/></Link><a title="匯出單筆" className="rounded-lg p-2 text-emerald-700 hover:bg-emerald-50" href={`/api/export/excel?id=${r.id}`}><Download size={17}/></a><button title="刪除" className="rounded-lg p-2 text-red-700 hover:bg-red-50" onClick={()=>remove(r)}><Trash2 size={17}/></button></div></td></tr>)}</tbody></table></div>
        <div className="divide-y lg:hidden">{records.map(r=><article key={r.id} className="p-4"><div className="mb-3 flex items-start justify-between"><div><span className="mr-2 rounded bg-slate-100 px-2 py-1 text-xs font-bold">{r.productionLine}</span><b>{r.productName}</b><p className="mt-2 text-xs text-slate-500">{formatDate(r.recordDate)} · {r.userName}</p></div><StatusBadge status={r.status}/></div><div className="grid grid-cols-2 gap-3 rounded-xl bg-slate-50 p-3 text-sm"><div><small className="text-slate-500">數量</small><b className="block">{r.quantityMaru} 丸 / {r.quantityBox} 箱</b></div><div><small className="text-slate-500">總生產時數</small><b className="block">{r.totalProductionText||'—'}</b></div></div><div className="mt-3 flex gap-2"><Link className="btn-secondary flex-1" to={`/admin/records/${r.id}/edit`}><Edit3 size={16}/>編輯</Link><a className="btn-secondary" href={`/api/export/excel?id=${r.id}`}><Download size={16}/></a><button className="btn-danger" onClick={()=>remove(r)}><Trash2 size={16}/></button></div></article>)}</div>
      </>}
    </section>
  </div>
}
